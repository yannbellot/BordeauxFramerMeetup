...
page.addPage(home) # Insertion de la première page
page.addPage(section_1, "bottom") # Insertion de la page 2 sous la page précédente
...
menu_home.onClick (event, layer) ->
	page.snapToPage(home) # Afficher à la page home
...
menu_section_1.onClick (event, layer) ->
	page.snapToPage(section_1)
	menu.animate
		height: 0
		options:
			time: 0.5
...
menu_section_2.onClick (event, layer) ->
...
menu_section_3.onClick (event, layer) ->
...
close_chatbox.opacity = 0
chatbot_pop.opacity = 0

chatbot.placeBefore(page)
chatbot_pop.placeBefore(page)
...
call_button.onClick (event, layer) ->
